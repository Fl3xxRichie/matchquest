...
class MatchQuest:
    ...
    def clear_terminal(self):
        ...
        print_banner()  # Show banner after clearing
        self.print_divider()
    ...
    def print_section_header(self, title):
        ...
        print(f"{Fore.YELLOW + Style.BRIGHT}╔{'═' * 78}╗{Style.RESET_ALL}")
        print(f"{Fore.YELLOW + Style.BRIGHT}║{' ' * padding}{Fore.WHITE + Style.BRIGHT}{title}{Fore.YELLOW + Style.BRIGHT}{' ' * (78 - len(title) - padding)}║{Style.RESET_ALL}")
        print(f"{Fore.YELLOW + Style.BRIGHT}╚{'═' * 78}╝{Style.RESET_ALL}")

    def print_timestamp(self, message):
        timestamp = f"[ {datetime.now().astimezone().strftime('%x %X %Z')} ]"
        print(
            f"{Fore.CYAN + Style.BRIGHT}{timestamp:<30}{Style.RESET_ALL}"
    ...
    def main(self, queries):
     while True:
        try:
            self.clear_terminal()  # This will now show the banner and divider
            
            # Registration Phase
            self.print_section_header("Registration Phase")
            ...
            # Home Section
            self.print_section_header("Home Dashboard")
                ...
                if 'code' in reward_point and reward_point['code'] == 200:
                    if reward_point['data']['next_claim_timestamp'] == 0:
                        ...
                        if datetime.now().astimezone() >= datetime.fromtimestamp(reward_point['data']['next_claim_timestamp'] / 1000).astimezone():
                            ...
                            self.print_timestamp(
                                f"{Fore.YELLOW + Style.BRIGHT}[Reward {int(reward_point['data']['reward'] / 1000)}] Farming Can Be Claimed At {datetime.fromtimestamp(reward_point['data']['next_claim_timestamp'] / 1000).astimezone().strftime('%x %X %Z')}{Style.RESET_ALL}"
                            )
                self.status_task_daily(token=token, first_name=first_name, uid=uid)

            # Gaming Section
            self.print_section_header("Gaming Status")
            for (token, first_name, uid) in accounts:
                self.rule_game(token=token, first_name=first_name)

            # Tasks Section
            self.print_section_header("Tasks Overview")
            for (token, first_name, uid) in accounts:
            ...
            self.print_timestamp(f"{Fore.MAGENTA + Style.BRIGHT}Total Accounts: {len(accounts)} | Total Balance: {total_balance}{Style.RESET_ALL}")

            if restart_times:
                now = datetime.now().astimezone().timestamp()
                future_farming_times = [time - now for time in restart_times if time > now]
                sleep_time = min(future_farming_times) + 30 if future_farming_times else 15 * 60
            else:
                sleep_time = 15 * 60

            next_restart = (datetime.now().astimezone() + timedelta(seconds=sleep_time)).strftime('%x %X %Z')
            self.print_timestamp(f"{Fore.YELLOW + Style.BRIGHT}Next Restart: {next_restart}{Style.RESET_ALL}")
            self.print_divider()

            sleep(sleep_time)
            
        except Exception as e:
            self.print_timestamp(f"{Fore.RED + Style.BRIGHT}Error: {str(e)}{Style.RESET_ALL}")
            continue
